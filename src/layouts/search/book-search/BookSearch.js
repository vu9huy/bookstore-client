...
const BookSearch = ({ searchKey, skip, limit, page, queryObj }) => {
    ...
    //     bookName: searchKey
    // }
    const { isLoading, data, error } = useQuery([`search-books-${searchKey}-${page}`], async () => await getBookByConditionApi(queryObj, skip, limit), { refetchOnWindowFocus: false, cacheTime: 600000, staleTime: Infinity })
